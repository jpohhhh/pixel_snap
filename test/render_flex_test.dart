...
void _setFlex(RenderBox child, int? flex) {
  final cpd = child.parentData as FlexParentData;
  cpd.flex = flex;
}
...
void main() {
  ...
  group('Pixel Snap', () {
    ...
    test('Cross-axis pixel snapped', () {
      ...
      // check if properly pixel snapped
      expect((box1.parentData as BoxParentData).offset.dy, 44);
      expect((box2.parentData as BoxParentData).offset.dy, 39);
    });

    test('Space Between', () {
      const constraints = BoxConstraints.tightFor(width: 13, height: 10);
      final box1 = RenderConstrainedBox(additionalConstraints: constraints);
      final box2 = RenderConstrainedBox(additionalConstraints: constraints);
      ...
      layout(
        flex,
        constraints: const BoxConstraints.tightFor(width: 46, height: 100),
      );

      expect((box1.parentData as BoxParentData).offset.dx, 0);
      expect((box2.parentData as BoxParentData).offset.dx, 16);
      expect((box3.parentData as BoxParentData).offset.dx, 33);
    });
    ...
    test('Space Around', () {
      ...
      layout(
        flex,
        constraints: const BoxConstraints.tightFor(width: 56, height: 100),
      );

      expect((box1.parentData as BoxParentData).offset.dx, 3);
      expect((box2.parentData as BoxParentData).offset.dx, 22);
      expect((box3.parentData as BoxParentData).offset.dx, 40);
    });
    ...
    test('Space Evenly', () {
      ...
      expect((box1.parentData as BoxParentData).offset.dx, 4);
      expect((box2.parentData as BoxParentData).offset.dx, 21);
      expect((box3.parentData as BoxParentData).offset.dx, 39);
    });
  });
